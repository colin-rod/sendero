...
function TourCard({ id, title, imageSrc, imageAlt, description, distance, elevation, elevationGain }: TourGridCardData) {
  ...
  return (
    <div
      className="group relative aspect-square w-full overflow-hidden cursor-pointer select-none touch-pan-y"
      onClick={() => {
    ...
    >
      ...
      <div className="absolute inset-0 flex flex-col justify-end items-center px-6 pb-[80px] lg:pb-[125px]">
        {(() => {
          ...
          return (
            <p className="text-center text-h3 font-light leading-8 tracking-wide text-background">
              {first && <>{first} </>}
